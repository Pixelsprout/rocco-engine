...
axis = |input, neg, pos| {
    n = if List.contains(input.held, neg) { 1.0 } else { 0.0 }
    p = if List.contains(input.held, pos) { 1.0 } else { 0.0 }
    p - n
}
...
steer = |m, input| map_entities(m, |e| match e.kind {
...
spin = |m, dt| map_entities(m, |e| match e.kind {
...
collect = |m| match player(m) {
    ...
    Ok(p) => {
        ...
        entities = List.map(m.entities, |e| if touched(e) { { ..e, alive: Bool.False } } else { e })
...
player = |m| List.find_first(m.entities, |e| match e.kind { Player => Bool.True, _ => Bool.False })

is_pickup : Entity -> Bool
is_pickup = |e| match e.kind { Pickup => Bool.True, _ => Bool.False }

door_open : Model -> Bool
door_open = |m| List.any(m.entities, |e| match e.kind { Door({ open }) => open, _ => Bool.False })
...
expect {
    ...
    on_top = map_entities(m, |e| if is_pickup(e) { { ..e, pos: origin } } else { e })
